...
namespace OpenCL
{
  ...
  char const* translate_cl_error(cl_int err)
  {
  ...
  (
    ...
    : m_platform(platform), device_count(dev_count),
      device_ids(dev_ids), m_context(cont)
  {}
  ...
  void Environment::release(void)
  {
      ...
      free((cl_device_id* const)device_ids);
      cout << "[Context] Closed." << endl;
  }

  RemoteBuffer Environment::allocate_space(size_t byte_size, void* bytes)
  {
      cl_mem remote_buffer = clCreateBuffer(m_context,
  ...
  cl_command_queue Environment::createCommandQueue(void)
  {
      cl_command_queue queue = clCreateCommandQueue(m_context, device_ids[0], 0, &error);
      if(error != CL_SUCCESS)
      {
  ...
  void writeBufferBlocking(cl_command_queue& queue, RemoteBuffer& remote, void* data)
  {
    error = clEnqueueWriteBuffer(queue, remote.buffer, CL_TRUE,
      0, remote.size, data, 0, nullptr, nullptr);
    if(error != CL_SUCCESS)
    {
  ...
  void readBufferBlocking(cl_command_queue& queue, RemoteBuffer& remote, void* data)
  {
    error = clEnqueueReadBuffer(queue, remote.buffer, CL_TRUE, 0,
      remote.size, data, 0, nullptr, nullptr);
    if(error != CL_SUCCESS)
    {
  ...
  Kernel::Kernel(string& fpath, string& mname)
  ...
  void Kernel::load(Environment& context)
  {
      ...
      m_program = clCreateProgramWithSource(context.m_context, 1,
  ...
  void Kernel::build(Environment& context)
  {
    error = clBuildProgram(
      m_program, context.device_count, context.device_ids,
    ...
    case CL_BUILD_PROGRAM_FAILURE:
      {
        ...
        size_t log_size;
        clGetProgramBuildInfo(m_program, context.device_ids[0],
  ...
  void Kernel::make(Environment& c)
  {
    load(c);
    build(c);
    create_kernel();
  ...
  {
      set_argument(nr, MEM_SIZE, &buf.buffer);
  }

  cl_event Kernel::enqueue_kernel(size_t width, cl_command_queue& queue)
  {
    const size_t workSize[] = { width, 0, 0 };
    cl_event event;
    error = clEnqueueNDRangeKernel(queue, m_kernel,
      1,
      nullptr,
      workSize,
      nullptr,
  ...
  cl_event Kernel::enqueue_kernel(size_t width, size_t height, cl_command_queue& queue)
  {
  ...
  template<typename T> void _print_device_info
  (
    cl_device_id device,
  ...
  Environment init(void)
  {
      ...
      cl_context context = clCreateContext(contextProperties,
        device_count, device_ids, nullptr, nullptr, &error);
      if(error != CL_SUCCESS)
      {
